...
/**
 * Поставщик данных, асинхронный вызов Callback и синхронный Future. Но так не нужно делать, это плохая практика
 *
 */
public class KafraProducer03 {
    // создание логера для класса
    private static final Logger logger = LoggerFactory.getLogger(KafraProducer03.class);
    ...
    public static void main(String[] args) {
        try (KafkaProducer<String, String> producer = new KafkaProducer<>(KafkaConfig.getProducerConfig())){
            for (int i=0; i<MAX_MSG; i++) {
                ...
                Future<RecordMetadata> result = producer.send(producerRec, (md, ex) ->{
                    if(ex != null){
